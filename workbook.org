...
In the Kitchin group, we use VASP to perform Density functional theory (DFT) calculations.
...
Neural networks are a machine learning technique which can be used to construct a flexible function from an arbitrary input variable.
...
#+label: fig-nn
#+caption: A basic neural network framework for a 2 atom system. Figure from cite:behler-2007-gener-neural.
...
#+label: fig-cutoff
#+caption: Demonstration of the cutoff radius in a 2D unit cell. Figure from cite:behler-2015-const.
#+attr_latex: :width 2in
#+attr_org: :width 200
[[./images/cutoff.png]]
...
- More information can be found in Reference citenum:behler-2007-gener-neural.
...
Now we have pseudo-randomly labeled 10% of our calculations for validation, and the rest are waiting to be trained in the new train.db file.

** Training neural networks

For all of out neural networks, we will be using the Behler-Parinello (BP) framework for distinguishing between geometries of atoms. Little to no work is published on how to systematically chose an appropriate number of variables for your BP framework, so we simply use the default settings in AMP for now. However, it is worth mentioning that a single G1 type variable (simplest possible descriptor) could be used to describe the fcc EOS, if that is all we are interested in.
